      ...
      <section class="half-page container">
        ...
        <hr />
          <b-form-group label="Nome:">
            <b-form-input type="text" size="lg" v-model="usuario.nome" placeholder="Seu nome"></b-form-input>
          </b-form-group>
          <b-form-group label="Email:">
            <b-form-input type="text" size="lg" v-model="usuario.email" placeholder="Seu email"></b-form-input>
          </b-form-group>
      ...
      <section class="half-page container">
        ...
        <hr />
          <b-list-group label="Nome:">
            ...
            :key="usuario.id">
              <div>{{usuario.id}} => {{usuario.nome}} => {{usuario.email}}</div>
              <b-button style="background-color:#666600"
               @click="editar(id)">Editar</b-button>
              <b-button variant="danger" @click="excluir(usuario.id)">Excluir</b-button>
            </b-list-group-item>
          </b-list-group>
      </section>
...
export default {
  ...
  methods:{
    salvar(){
      const vm = this
      const newRequest = !vm.usuario.id ? {method: vm.$http.post, item: 'usuarios.json'}  : {method: vm.$http.patch, item: `/usuarios/${vm.usuario.id}.json`}
